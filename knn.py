...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
      for j in range(num_test):
        ...
        test_sample = x_test[j]
        dist = (train_sample - test_sample) ** 2
        dists[i, j] = torch.sqrt(dist.sum())
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_test = x_test.view(num_test, -1)
    for j in range(num_test):
      # x_test[j] -> 1,d - x_train,d
     
      dist = (x_test[j] - x_train) ** 2
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    # num_train,1
    a2 = x_train**2
    a2 = a2.sum(dim=-1,keepdims=True)
    # 1,num_test 
    b2 = x_test**2
    b2 = b2.sum(dim=-1,keepdims=True).t()
    # m,n 
    ab2 = 2* x_train @ x_test.t()
    dist = a2+b2-ab2 
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    # Replace "pass" statement with your code
    # nearest 
    _,indices = torch.topk(dists,dim=0,k=k,largest=False)
    y_pred = y_train[indices]
    y_pred,_ = torch.mode(y_pred,dim=0)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
class KnnClassifier:
    ...
    def __init__(self, x_train: torch.Tensor, y_train: torch.Tensor):
        ...
        self.compute_dists = compute_distances_no_loops
    ...
    def predict(self, x_test: torch.Tensor, k: int = 1):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        dists = self.compute_dists(self.x_train,x_test)
        y_test_pred = self.predict_labels(dists,self.y_train,k)
        ######################################################################
        #                         END OF YOUR CODE                           #
        ######################################################################
        return y_test_pred
...
def knn_cross_validate(
    ...
    # Replace "pass" statement with your code
    # val_set change
    length = len(x_train_folds)
    for i in range(length):
      x_val = x_train_folds[i]
      y_val = y_train_folds[i]
      x_train = torch.cat([x_train_folds[j] for j in range(length) if j!=i])
      y_train = torch.cat([y_train_folds[j] for j in range(length) if j!=i])
      for k in k_choices:
        model = KnnClassifier(x_train,y_train)
        accuracy = model.check_accuracy(x_val,y_val,k=k,quiet=True)
