...
def svm_loss_vectorized(
    ...
    # N
    correct_class_scores = scores[indices,y[indices]].unsqueeze(dim=1)
    margin = scores - correct_class_scores + 1
    # don't compute the y[i]!
    margin[indices,y[indices]] = 0
    ...
    #############################################################################
    # Replace "pass" statement with your code
    # dW is d,c too. X is N,C, margin[N,C]
    # if margin>0, every sample give a X[i] 
    # N,D
   
    mask = (margin>0).double()
    incorrect_counts = torch.sum(mask,dim=1) # N,
    mask[torch.arange(num_train), y] -= incorrect_counts
...
def sample_batch(
    ...
    # Replace "pass" statement with your code
    data_nums = X.shape[0]
    indices = torch.randint(0,data_nums,(batch_size,))
    X_batch = X[indices]
    y_batch = y[indices]
    #########################################################################
    #                       END OF YOUR CODE                                #
...
def softmax_loss_vectorized(
    ...
    num_train=X.shape[0]
    scores = X @ W
    shift_scores=scores-scores.max(dim=1,keepdim=True)[0]
    softmax_output=torch.exp(shift_scores)/torch.sum(torch.exp(shift_scores),dim=1,keepdim=True)
    correct_class_probabilities=softmax_output[torch.arange(num_train), y]
    loss=-torch.sum(torch.log(correct_class_probabilities))/num_train
    loss+=reg*torch.sum(W*W)

    # compute gradient of the loss
    softmax_output[torch.arange(num_train), y]-=1
    dW=X.t() @ softmax_output / num_train
    dW+= 2*reg*W
    #############################################################################
    #                          END OF YOUR CODE                                 #
    #############################################################################
