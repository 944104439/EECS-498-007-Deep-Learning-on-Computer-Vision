...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    hidden = torch.maximum(torch.tensor(0), X @ W1 + b1)
    scores = hidden @ W2 + b2 
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_forward_backward(
    ...
    exp_scores_sum = torch.sum(exp_scores,dim=1,keepdim=True)
    softmax = exp_scores/exp_scores_sum
    log_softmax = torch.log(softmax)
    correct_prob = -log_softmax[torch.arange(N),y]
    data_loss = correct_prob.mean()
...
def find_best_net(
    ...
    for a in learning_rates:
      for b in regularization_strengths:
        for c in learning_rate_decays:
          for d in hidden_sizes:
            # print(a,b,c,d)
            model = TwoLayerNet(input_size, d, 10)
            stats = model.train(X_train, y_train, X_val, y_val,num_iters=3000, batch_size=1000,
                         learning_rate=a, learning_rate_decay=c, reg=b, verbose=False)
            y_pred = model.predict(X_val)
            val = (y_val==y_pred).float().mean().item()
